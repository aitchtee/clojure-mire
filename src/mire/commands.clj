...
(defn grab
  ...
  [thing]
  (dosync
    (if (or (= thing "money") (= thing "bagmoney5") (= thing "bagmoney10") (= thing "bagmoney20"))
      (if (room-contains-gold? @*current-room* thing)
        (do
          (case thing
            "money" (alter *money* inc)
    ...
    (dosync
      ...
      (case thing
        ...
        "bagmoney5" (if (> @*money* 4)
                      (do
                        (alter *money* - 5)
                        (alter (:gold @*current-room*) conj (keyword thing))
        ...
        (if (carrying? thing)
          (do
            (move-between-refs (keyword thing) *inventory* (:items @*current-room*))
